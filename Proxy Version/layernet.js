...
const { HttpsProxyAgent } = require('https-proxy-agent'); // Updated import

const uri = 'wss://tongame-service-roy7ocqnoq-ew.a.run.app/socket.io/?EIO=4&transport=websocket';
const options = (accessToken, proxy) => {
    const proxyUrl = `http://${proxy.trim()}`;
    const agent = new HttpsProxyAgent(proxyUrl);
    console.log(`Using proxy: ${proxyUrl}`); // Log the proxy being used
    return {
        headers: {
...
async function connect(accessToken, accountNumber, resolve) {
    ...
    const proxyHost = proxy.trim().replace(/^http:\/\//, ''); // Remove http prefix and trim any whitespace
    console.log(`Connecting with proxy: ${proxyHost}`); // Log the selected proxy
  
    function attachWebSocketHandlers() {
        ws.on('open', function open() {
            logMessage(accountNumber, `Account #${accountNumber} | WebSocket connection opened for token: ${accessToken} | Proxy: ${proxyHost}`);
            reconnectAttempts = 0;
                ...
                try {
                    if (content.startsWith('[') || content.startsWith('{')) {
                        ...
                        if (json[0] && json[0].duration) {
                            ...
                            logMessage(accountNumber, `Account #${accountNumber} | Proxy: ${proxyHost} | Durability: ${gameData.durability} | Gold: ${gameData.gold} | Rank: ${userRankData.rank} | Per Hour: ${userRankData.profitPerHour} | Dogs: ${userRankData.dogs}`);
    ...
    ws = new WebSocket(uri, options(accessToken, proxyHost)); // Use proxyHost in options
